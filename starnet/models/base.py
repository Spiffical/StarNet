...
class BaseModel(object):
    ...
    def compile_model(self):
        ...
        # If there is no Keras model defined, instantiate one
        if self.keras_model is None:
            print('[INFO] Instantiating Keras model')
            if self.num_gpu > 1:
                with tf.device('/cpu:0'):  # Have the weights (shared amongst all GPUs) stored on the CPU for updating
        ...
        # Compile model
        if self.num_gpu > 1:
            print('[INFO] Compiling multi-GPU Keras model')
            self.mgpu_keras_model = multi_gpu_model(self.keras_model, self.num_gpu)
            self.mgpu_keras_model.compile(loss=self.loss_func, optimizer=self.optimizer,
                                          metrics=self.metrics, loss_weights=None)
        else:
            print('[INFO] Compiling Keras model')
            self.keras_model.compile(loss=self.loss_func, optimizer=self.optimizer,
                                     metrics=self.metrics, loss_weights=None)
    ...
    def pre_training_checklist(self):
        ...
        self.wav = self.get_wave_grid()  # get the wave grid
        ...
        checkpoint = CustomModelCheckpoint(filepath, self.keras_model, monitor='val_loss', verbose=1,
...
class BaseDeepEnsemble(BaseModel):

    def __init__(self):
        super(BaseDeepEnsemble, self).__init__()
        self._model_type = 'DeepEnsemble'
        self.loss_func = gaussian_loss
        self.deepensemble_sigma = None
    ...
    def compile_model(self):
        ...
        if self.keras_model is None:
            ...
            if self.num_gpu > 1:
                with tf.device('/cpu:0'):  # Have the weights (shared amongst all GPUs) stored on the CPU for updating
                    self.keras_model, self.deepensemble_sigma = self.model()
            else:
                self.keras_model, self.deepensemble_sigma = self.model()

        # TODO: checkpointing multi-gpu model does not currently work for Deep Ensembles
        if self.num_gpu > 1:
            print('[INFO] Compiling multi-GPU Keras model')
            self.mgpu_keras_model = multi_gpu_model(self.keras_model, self.num_gpu)
            self.mgpu_keras_model.compile(loss=self.loss_func(self.deepensemble_sigma), optimizer=self.optimizer,
                                          metrics=self.metrics, loss_weights=None)
        else:
            print('[INFO] Compiling Keras model')
...
class BaseDeepEnsembleTwoModelOutputs(BaseModel):
    """
    This is for an implementation of the Deep Ensemble method in which the model takes two inputs (features
    and labels) and has two outputs (mean and sigma). In this implementation, the loss is defined in the
    model() function and so shouldn't be defined when compiling the model.
    """

    def __init__(self):
        super(BaseDeepEnsembleTwoModelOutputs, self).__init__()
        self._model_type = 'DeepEnsemble'

    def pre_training_checklist(self):

        super().pre_training_checklist()
    ...
    def compile_model(self):
        ...
        if self.keras_model is None:
            ...
            if self.num_gpu > 1:
                #with tf.device('/cpu:0'):  # Have the weights (shared amongst all GPUs) stored on the CPU for updating
                model_ = self.model()
                self.keras_model = multi_gpu_model(model_, self.num_gpu)
                self.mgpu_keras_model = multi_gpu_model(model_, self.num_gpu)
            else:
                self.keras_model = self.model()
                self.keras_model = multi_gpu_model(self.keras_model, self.num_gpu)

        if self.num_gpu > 1:
            print('[INFO] Compiling multi-GPU Keras model')
            self.mgpu_keras_model.compile(optimizer=self.optimizer,
                                          metrics=self.metrics, loss_weights=None)
        else:
            print('[INFO] Compiling Keras model')
