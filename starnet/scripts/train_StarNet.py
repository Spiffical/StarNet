...
parser.add_argument('--masks', type=str, default=None,
                    help='Path of numpy file which contains masks to apply to all spectra')
...
global_mask_path = args.global_mask
...
if not global_mask_path or global_mask_path is None or global_mask_path == 'None':
    pass
else:
    NN.global_err_mask = np.load(global_mask_path)

if not masks_path or masks_path is None or masks_path == 'None':
    pass
else:
    NN.masks = np.load(masks_path)
