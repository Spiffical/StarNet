...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, indices, data_filename, targetname, mu, sigma, spec_name, min_flux_value=0, 
                 max_flux_value=1.03, max_added_noise=None, normalized_spec=True, max_fraction_zeros=None, 
    ...
    def augment_spectra(self, X):
        ...
        # Normalize spectra if not already (this takes awhile, and ideally shouldn't be done on the fly)
        if not self.normalized_spec:
            for x in X:
                x, _, _ = continuum_normalize(x, self.line_regions, self.wav, self.segments_step)
